...
function asIsoDate(v: string | undefined) {
  if (!v) return "";
  return /^\d{4}-\d{2}-\d{2}$/.test(v) ? v : "";
}

function monthRange(v: string | undefined) {
  if (!v || !/^\d{4}-\d{2}$/.test(v)) return { start: "", endExclusive: "" };
...
export async function getStatsTableData(
  ...
  if (table === "visits") {
    const [countRow = {}] = await sql`
      ...
      WHERE
        ...
        AND (${month.start} = '' OR (created_at >= ${month.start}::date AND created_at < ${month.endExclusive}::date));
    `;
  ...
  const [countRow = {}] = await sql`
    ...
    WHERE
      ...
      AND (${dateFrom} = '' OR s.last_seen_at >= ${dateFrom}::date)
      AND (${dateTo} = '' OR s.last_seen_at < (${dateTo}::date + INTERVAL '1 day'))
      AND (${month.start} = '' OR (s.last_seen_at >= ${month.start}::date AND s.last_seen_at < ${month.endExclusive}::date));
  `;
  const total = num((countRow as { count?: string | number }).count);
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const page = Math.min(Math.max(1, pageRequested), totalPages);
